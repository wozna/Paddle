...
void CPUBFloat16Pass::SetInputDataType(ir::Graph* graph) const {
  ...
  gpd(graph, handler);
  PrettyLogDetail("---    added %d quantize op before bfloat16 op", quantize_counter);
}
...
void CPUBFloat16Pass::SetOutputDataType(ir::Graph* graph) const {
  ...
  gpd(graph, handler);
  PrettyLogDetail("---    added %d dequantize op and used %d force_fp32_output", dequantize_counter, force_fp32_counter);
}
