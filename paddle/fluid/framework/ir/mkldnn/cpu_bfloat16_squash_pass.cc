...
#include "paddle/fluid/framework/ir/mkldnn/cpu_bfloat16_squash_pass.h"
...
void CPUBfloat16SquashPass::FindNodesToKeep(
    Graph* graph,
...
void CPUBfloat16SquashPass::DequantQuantSquash(
    Graph* graph,
    std::unordered_map<const Node*, int>* nodes_keep_counter) const {
  GraphPatternDetector gpd;
...
// conv2d, fc, matmul
void CPUBfloat16SquashPass::OpDequantSquash(Graph* graph) const {
  GraphPatternDetector gpd;
...
void CPUBfloat16SquashPass::MultipleQuantizeSquash(Graph* graph) const {
  GraphPatternDetector gpd;
...
void CPUBfloat16SquashPass::DequantScaleSquash(Graph* graph) const {
  GraphPatternDetector gpd;
...
void CPUBfloat16SquashPass::ApplyImpl(ir::Graph* graph) const {
  PADDLE_ENFORCE_NOT_NULL(
      graph,
      platform::errors::InvalidArgument(
          "The graph in function CPUBfloat16SquashPass::ApplyImpl is null."));
...
REGISTER_PASS(cpu_bfloat16_squash_pass,
              paddle::framework::ir::CPUBfloat16SquashPass);
