 ...
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    if (reorder_p == nullptr) {
      ...
      if (bfloat16) {
        platform::SetDstMemoryQuantized<paddle::platform::bfloat16>(ctx, output, dst_tz,
                                         engine, dst_md, dst_memory, out_format);
      } else if (is_negative) {
      ...
      if (bfloat16) {
        dst_memory->set_data_handle(output->mutable_data<paddle::platform::bfloat16>(place));
      } else if (is_negative) {
