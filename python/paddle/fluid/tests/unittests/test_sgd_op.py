...
class TestSGDOp(OpTest):
    def setUp(self):
        ...
        self.conf()
        self.use_bf16 = False
        self.dt = "uint16" if self.use_bf16 else "float32"

        w = np.random.random((self.h, self.w)).astype(self.dt)
        g = np.random.random((self.h, self.w)).astype(self.dt)
        # w = np.random.random((self.h, self.w)).astype("float32")
        # g = np.random.random((self.h, self.w)).astype("float32")
        # lr = np.array([0.1]).astype("uint16")
...
class TestSparseSGDOp(unittest.TestCase):
    def check_with_place(self, place):
        ...
        np_array = np.ones((len(rows), self.row_numel)).astype(self.dt)
        np_array[0, 0] = 2.5
        np_array[2, 8] = 4.5
        if self.use_bf16:
            np_array = amp.convert_float_to_uint16(np_array)
        ...
        param_array = np.full((height, self.row_numel), 5.5).astype(self.dt)
        if self.use_bf16:
            param_array = amp.convert_float_to_uint16(param_array)
        param.set(param_array, place)

        # create and initialize LeraningRate Variable
        lr = scope.var('LearningRate').get_tensor()
        if self.use_bf16:
            lr_array = np.full((1), 2.0).astype(self.dt)
        if self.use_bf16:
            lr_array = amp.convert_float_to_uint16(lr_array)
        ...
        if self.use_bf16:
            result_array = convert_uint16_to_float(result_array)
